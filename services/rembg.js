...
async function removeBackground(inputPath, outputPath) {
  // 1. 阿里云（主力）
  if (alibaba) {
    try { var r = await alibaba.removeBackground(inputPath, outputPath); if (r) { console.log("阿里云 抠图成功"); return r; } } catch(e) {}
  }
  // 2. 百度 AI（降级）
  if (baiduAI) {
    try { var r = await baiduAI.removeBackground(inputPath, outputPath); if (r) { console.log("百度AI 抠图成功"); return r; } } catch(e) {}
  }
  // 3. Sharp 兜底
...
async function replaceBackground(inputPath, bgColor, outputPath) {
  // 1. 阿里云
  if (alibaba) {
    try { var r = await alibaba.replaceBackground(inputPath, bgColor, outputPath); if (r) { console.log("阿里云 换背景成功"); return r; } } catch(e) {}
  }
  // 2. 百度 AI
  if (baiduAI) {
    try { var r = await baiduAI.replaceBackground(inputPath, bgColor, outputPath); if (r) { console.log("百度AI 换背景成功"); return r; } } catch(e) {}
  }
  // 3. 本地合成
