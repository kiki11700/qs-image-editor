...
let alibaba = null;
try { alibaba = require("./alibaba"); } catch(e) { console.error("������ AI ����ʧ��:", e && e.message || "unknown"); }
...
async function upscale(inputPath, outputPath) {
  if (alibaba) {
    try { var r = await alibaba.upscale4K(inputPath, outputPath); if (r) { console.log("������ ���ֳɹ�"); return r; } } catch(e) { console.error("������ AI ����ʧ��:", e && e.message || "unknown"); }
  }
...
async function upscaleTo4K(inputPath, outputPath) {
  if (alibaba) {
    try { var r = await alibaba.upscale4K(inputPath, outputPath); if (r) { console.log("������ ת4K�ɹ�"); return r; } } catch(e) { console.error("������ AI ����ʧ��:", e && e.message || "unknown"); }
  }
  var meta = await sharp(inputPath).metadata();
...
async function styleTransfer(inputPath, stylePrompt, outputPath) {
  if (alibaba) {
    try { var r = await alibaba.styleTransfer(inputPath, stylePrompt, outputPath); if (r) { console.log("������ ���Ǩ�Ƴɹ�"); return r; } } catch(e) { console.error("������ AI ����ʧ��:", e && e.message || "unknown"); }
  }
  var pt = stylePrompt || "anime style";
  var px = pt.toLowerCase();
  if (px.includes("�ڰ�") || px.includes("bw") || px.includes("�Ҷ�")) { await sharp(inputPath).greyscale().png().toFile(outputPath); return outputPath; }
  var sat = 1.1, gamma = 1.0, blur = false, tint = { r: 0, g: 0, b: 0 };
  if (px.includes("����") || px.includes("anime") || px.includes("��ͨ")) { sat = 1.3; gamma = 1.111; tint.r = 10; tint.g = 5; tint.b = 15; }
  else if (px.includes("�ͻ�") || px.includes("oil")) { sat = 1.2; gamma = 1.176; tint.r = 5; tint.g = 5; }
  else if (px.includes("ˮ��") || px.includes("water")) { sat = 0.9; gamma = 1.1; blur = true; tint.b = 20; }
  else if (px.includes("����") || px.includes("vintage") || px.includes("����")) { sat = 0.8; gamma = 1.111; tint.r = 30; tint.g = 15; }
  else if (px.includes("����") || px.includes("cyber") || px.includes("neon")) { sat = 1.5; gamma = 1.053; tint.b = 30; }
...
async function generateSimilar(inputPath, outputPath) {
  if (alibaba) {
    try { var r = await alibaba.generateSimilar(inputPath, outputPath); if (r) { console.log("������ ������ͼ�ɹ�"); return r; } } catch(e) { console.error("������ AI ����ʧ��:", e && e.message || "unknown"); }
  }
