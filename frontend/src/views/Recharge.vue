...
<h2 class="text-2xl font-bold text-center mb-2">购买额度</h2>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
      ...
      >
        ...
        <div class="text-xs text-gray-500 mb-3">修图次数</div>
        <div class="text-lg font-bold text-white">¥{{ pkg.price }}</div>
        <div class="text-xs text-gray-500 mt-1">约 ¥{{ (pkg.price / pkg.credits).toFixed(2) }}/次</div>
      </div>
    </div>

    <div class="bg-white/[0.04] rounded-xl p-5 border border-white/[0.08] mb-4">
      <div class="text-sm font-medium mb-3">兑换码</div>
      <div class="flex gap-2">
        <el-input v-model="code" placeholder="输入兑换码" size="large" class="flex-1" />
        <el-button type="primary" size="large" @click="redeem">兑换</el-button>
      </div>
    </div>
    ...
    <div class="flex gap-3 justify-center">
      <el-button type="primary" size="large" round :disabled="!selectedPkg || paying" :loading="paying" @click="pay" class="!px-12 !text-base">
        {{ paying ? "支付中..." : "立即支付 ¥" + (selectedPkg ? packages.find(p=>p.id===selectedPkg)?.price : 0) }}
      </el-button>
    </div>
  </div>
</template>
...
async function pay() {
  ...
  try {
    ...
    ElMessage.success("充值成功！获得 " + res.data.credits + " 次额度");
  } catch (e) {
...
async function redeem() {
  ...
  if (!code.value) { ElMessage.warning("请输入兑换码"); return; }
  try {
