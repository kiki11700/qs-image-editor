...
router.post("/process", authMiddleware, upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      dbRun("UPDATE tasks SET status = ?, output_path = ?, completed_at = CURRENT_TIMESTAMP WHERE id = ?", ["completed", outputPath, taskId]);
...
router.get("/task/:id", authMiddleware, (req, res) => {
  ...
  res.json({ id: task.id, type: task.type, status: task.status, createdAt: task.created_at, completedAt: task.completed_at, outputUrl: task.status === "completed" ? "/api/image/download/" + task.id : null });
...
router.get("/tasks", authMiddleware, (req, res) => {
  ...
  res.json(tasks.map(t => ({ id: t.id, type: t.type, status: t.status, createdAt: t.created_at, completedAt: t.completed_at, outputUrl: t.status === "completed" ? "/api/image/download/" + t.id : null })));
