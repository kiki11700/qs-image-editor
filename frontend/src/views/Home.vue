      ...
      <template v-if="selectedTool">
        <div v-if="selectedToolDef?.needsParam" class="mb-5">
          ...
          <div v-if="selectedToolDef.paramType === 'text'" class="max-w-md">
            <input v-model="extraParam" :placeholder="selectedToolDef.paramPlaceholder || '输入描述...'"
          ...
          <div v-else-if="selectedToolDef.paramType === 'color'" class="flex items-center gap-3">
        ...
        <div @drop.prevent="handleDrop" @dragover.prevent
          ...
          <input type="file" accept="image/*" class="hidden" id="fileInput" @change="handleFile($event.target.files[0])" />
    ...
    <div v-if="resultVisible" class="mt-6 bg-white/[0.03] rounded-2xl border border-white/[0.08] p-6 sm:p-8">
      ...
      <div class="flex justify-center gap-3 mt-4">
        <button @click="downloadResult"
          class="px-6 py-2.5 rounded-xl text-sm font-bold bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white hover:shadow-lg transition-all">
          &#x2B07; 下载结果
        </button>
        <button @click="resetAll"
          class="px-6 py-2.5 rounded-xl text-sm font-medium border border-white/[0.15] text-gray-300 hover:bg-white/[0.05] transition-all">
          继续处理
        </button>
      </div>
    </div>
...
const tools = [
  { id: "upscale",         icon: "\uD83D\uDDBC\uFE0F", color: "#667eea", color2: "#764ba2", name: "转高清",     desc: "AI 增强画质，提升分辨率" },
  { id: "upscale4k",       icon: "\uD83D\uDCFA", color: "#e74c3c", color2: "#c0392b", name: "转 4K 高清", desc: "极致超清化处理，输出 4K 图片" },
  { id: "vectorize",       icon: "\uD83D\uDCD0", color: "#2ecc71", color2: "#27ae60", name: "转矢量图",   desc: "位图转 SVG 矢量，无限放大不失真" },
  { id: "remove-bg",       icon: "\u2702\uFE0F", color: "#3498db", color2: "#2980b9", name: "抠图去底",   desc: "AI 自动识别主体，一键去除背景" },
  { id: "replace-bg",      icon: "\uD83C\uDFA8", color: "#f39c12", color2: "#e67e22", name: "更换背景",   desc: "更换背景颜色或图片", needsParam: true, paramType: "color", paramLabel: "背景颜色" },
  { id: "style-transfer",  icon: "\u2728", color: "#9b59b6", color2: "#8e44ad", name: "转风格",     desc: "AI 风格迁移，创意无限", needsParam: true, paramType: "text", paramLabel: "风格描述", paramPlaceholder: "例如: 动漫风格、油画风格、水彩风格" },
  { id: "similar",         icon: "\uD83D\uDD00", color: "#1abc9c", color2: "#16a085", name: "出类似图",   desc: "AI 生成相似风格的全新图片" },
  { id: "extract-pattern", icon: "\uD83D\uDD8C\uFE0F", color: "#e84393", color2: "#c44569", name: "印花提取",   desc: "从衣服/布料提取印刷图案，支持透明背景输出" },
];
...
function toolCardClass(t) {
  if (selectedTool.value !== t.id) {
    return 'bg-white/[0.04] border-white/[0.08] hover:border-white/[0.2] hover:bg-white/[0.06]';
  }
  const c1 = t.color || '#667eea';
  const c2 = t.color2 || '#764ba2';
  return 'bg-gradient-to-br from-' + c1 + '/20 to-' + c2 + '/20 border-' + c1 + ' shadow-lg';
}

const processBtnClass = computed(() => {
  if (!canProcess.value || processing.value) {
    return 'bg-gray-700 text-gray-500 cursor-not-allowed';
  }
  return 'bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white hover:shadow-lg hover:shadow-[#667eea]/20 active:scale-[0.98]';
});
